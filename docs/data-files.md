...
|---|---|---|
| `application.json` | `jobapply new`, applicant | `company`, `role`, `language`, `posting_url`, `form_url` (a web Form, or `mailto:<address>` when the employer takes applications by email: then `scan`/`fill` give way to `email`), `attachments` (files from `attachments/manifest.json` to upload, in order), `profile_overrides` (deep-merged over `profile.json` for this Application only), `style` / `letter_style` (Style names for the CV and the letter, override `config.json`; the letter falls back to the CV's), `cover_letter` (`false` waives the letter: the `letter` step counts as done, `render` produces only CV and Dossier, `document:cover_letter` may not be used) |
