...
| `email.md` | `jobapply email` | By-email Applications only: the covering message as composed — `To`, `Subject` (as on the letter), salutation, the fixed `email_body` paragraphs from `cover-letter.<lang>.json` (`{role}`, `{company}`, `{reference}`), sign-off, name and phone, and the file to attach (the Dossier). The tool hands To/Subject/body to the mail client through a `mailto:` URL; attaching and sending is the applicant's |
