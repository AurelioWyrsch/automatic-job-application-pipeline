...
| `cover-letter.json` | applicant | The specific half of the letter: `draft` (must be `false` before render; `[d]` in `run` sets it), `subject`, `salutation` (empty = built from `posting.contact` via `salutation_named` in `cover-letter.<lang>.json`, or `salutation_default` when no contact is known), `intro` (paragraphs, markdown), `body` (paragraphs, markdown). Letter order: `intro` → fixed `about_me` → `body` → fixed `closing`. |
