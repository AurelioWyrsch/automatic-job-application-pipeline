...
2. Write in the Application's language. Set `register` to match the Posting: `formal` (Sie/"Dear") unless the Posting itself addresses the reader informally (Du), then `informal`; the fixed half switches its own wording accordingly, so keep the specific half consistent with that choice. Use the applicant's own facts from `profile.json`; invent nothing.
...
4. Show the draft to the applicant in the reply as well, then open the file for them: `jobapply open <slug> letter`.
