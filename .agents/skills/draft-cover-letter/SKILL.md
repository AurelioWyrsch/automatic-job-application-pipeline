...
4. Write plain prose: paragraphs of running text, the way a letter is typed, with the applicant's answers folded into sentences. Markdown stays out of the letter (no lists, no bold). Write in the Application's language, addressing the reader formally (Sie/"Dear") whatever tone the Posting uses. Use only the sentences confirmed in step 3; invent nothing.
5. Produce, in `cover-letter.json`:
   ...
   - `intro`: one paragraph, starting with a capital letter (the salutation ends with a comma), that names the role and carries the confirmed sentence that fits best.
   - `body`: one or two paragraphs that take the remaining confirmed sentences in turn, each tying the Posting's wording to the applicant's evidence. End with one sentence on why this employer.
